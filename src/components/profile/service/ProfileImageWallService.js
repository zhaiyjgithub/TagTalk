...
export const GetImageWalls = (chatId, success, fail) => {
	const param = {
		ChatID: chatId
	}

	HTTP.post(API_User.GetImageWall, param).then((response) => {
...
const preHandleNewImage = (chatId, allImages: Array) => {
	let newFiles = []
	let allFileNames = []
...
export const UpdateImageWalls = (chatId, allImages, deleteImages, success, fail) => {
	let uploadUrl = 'http://localhost:8090/User/UploadImageWalls';
	let newImageInfo = preHandleNewImage(chatId, allImages)
